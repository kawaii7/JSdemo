...
$(function () {
    ...
    function init() {
        ...
        //当页面一加载就触发next按钮的点击事件，用trigger的好处是减少重复代码,如果不用，
        // 就要把按钮click事件里代码全部复制过来，因为加载的代码和点击next按钮代码和效果相同，
        // 所以就用trigger触发执行click里面代码。
    }
    ...
    function showCurrent(l) {
        ...
        $("ul").animate({"left": l}, 500, function () {
            if (l == 0) {
                ...
                // 我在做的时候，用的不是css,而是用animate()定位到第二个第一张，结果切换时，
                // 是反方向的运动，一直觉得无论怎样，图片方向都会发生变化 ，弄得花了一天时间才找到原因，所以一定要用css。
                $("ul").css("left", -len * w / 2);
            } else if (l == (1 - len) * w) {
